...
if (args === '0') {
                                        ...
                                        const role = message.guild.roles.cache.find(r => r.name === 'Muted');
                                         user.roles.add(role.id).catch(console.error).then(
                                        user.send(`You have been muted from ${message.guild.name} for ${ms(ms(time), { long: true })} because of: ${reason}`),
                                        message.say(`**${user}** has been muted for ${ms(ms(time), { long: true })} because of: ${reason}`),
                                        );
                                        }
